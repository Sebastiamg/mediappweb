        ...
        <nav className="bg-white border-gray-200 py-2.5 dark:bg-gray-900">
          <div className="flex flex-wrap items-center justify-between max-w-screen-xl px-4 mx-auto">
            <Link to={"/"} className="flex items-center">
              ...
              {data.user && (
                <span className="text-slate-500 ml-1 uppercase text-sm">
                  {" "}
                  | {data.user.role?.name}
                </span>
              )}
            ...
            <div className="flex items-center lg:order-2">
              ...
              {data.isAuthenticated && (
                <Link
                  to={"/user/profile"}
                  className="block py-2 pl-3 mr-16 text-white hover:text-purple-500 bg-purple-700 rounded lg:bg-transparent lg:text-gray-200 lg:p-0 dark:text-white font-bold"
                  aria-current="page"
                >
                  {data.user.firstname}
                </Link>
            ...
            >
              <ul className="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0">
                ...
                {data.user && data.user.role?.name === "admin" && (
                  <li>
                    <Link
                      to={"/medic/create"}
                ...
                {data.user && data.user.role?.name === "medic" && (
                  <li>
                    <Link
                      to={"/appointment/gestion"}
                      className="block py-2 pl-3 pr-4 text-gray-700 border-b border-gray-100 hover:bg-gray-50 lg:hover:bg-transparent lg:border-0 lg:hover:text-purple-700 lg:p-0 dark:text-gray-400 lg:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white lg:dark:hover:bg-transparent dark:border-gray-700"
                    >
                ...
                {data.user && data.user.role?.name === "pacient" && (
                  <li>
                    <Link
                      to={"/appointment"}
                      className="block py-2 pl-3 pr-4 text-gray-700 border-b border-gray-100 hover:bg-gray-50 lg:hover:bg-transparent lg:border-0 lg:hover:text-purple-700 lg:p-0 dark:text-gray-400 lg:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white lg:dark:hover:bg-transparent dark:border-gray-700"
